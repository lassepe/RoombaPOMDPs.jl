...
function create_eval_plot(data=nothing;
                          filename::String="$(@__DIR__)/../results/final_results/data_eval_final.csv",
                          outdir::String="$(@__DIR__)/../results/final_results/plots/")

    for reward_type in [:inf_discounted_reward, :undiscounted_reward]
        if isnothing(data)
